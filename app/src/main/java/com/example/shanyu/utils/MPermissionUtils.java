...
public class MPermissionUtils {
    ...
    public static void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        if(requestCode == mRequestCode){
            if(verifyPermissions(grantResults)){   //已经授权
                if(mOnPermissionListener != null)
                    mOnPermissionListener.onPermissionGranted();
            }else{                                   //拒绝授权
                if(mOnPermissionListener != null)
    ...
    public static void showTipsDialog(final Context context) {
        MyCommonDialog dialog = new MyCommonDialog(context);
        dialog.setDialogContent("为了更好的提供服务，请前往设置中心进行权限授权");
        dialog.setOnSetPositiveButton("确定", new MyCommonDialog.onSetPositiveButton() {
            @Override
        ...
        dialog.setOnSetNegativeButton("取消", new MyCommonDialog.onSetNegativeButton() {
            @Override
